...
describe('FuzzyMatch', () => {
  describe('clean()', () => {
    it('should remove numeric prefixes', () => {
      ...
      expect(FuzzyMatch.clean('4_attire')).toBe('attire');
      expect(FuzzyMatch.clean('070_look')).toBe('look');
    });
    ...
    it('should handle combined cases', () => {
      ...
      expect(FuzzyMatch.clean('4_attire-formal')).toBe('attireformal');
    });
  });
  ...
  describe('findBest() with objects', () => {
    ...
    it('should ignore numeric prefixes', () => {
      const result = FuzzyMatch.findBest(sections, 'look', s => s.name);
      expect(result.name).toBe('070_look');
    });
  });
  ...
  describe('Real-world file matching scenarios', () => {
    ...
    it('should match attire files', () => {
      expect(FuzzyMatch.findBest(manifestFiles, 'cozy')).toBe('cozy_morning.md');
      expect(FuzzyMatch.findBest(manifestFiles, 'daddy')).toBe('daddys_girl_outfit.md');
      expect(FuzzyMatch.findBest(manifestFiles, 'office')).toBe('office_professional.md');
    });

    it('should match hair files', () => {
      expect(FuzzyMatch.findBest(manifestFiles, 'messy')).toBe('messy_bun.md');
      expect(FuzzyMatch.findBest(manifestFiles, 'ponytail')).toBe('high_ponytail.md');
    });
  });
});