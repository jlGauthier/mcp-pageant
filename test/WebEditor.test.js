...
async function createTestDirectories(tmpDir) {
  ...
  await fs.mkdir(path.join(mainManifest, '070_look', '4_attire', 'casual'), { recursive: true });
  ...
  await fs.writeFile(
    path.join(mainManifest, '070_look', '4_attire', 'casual', 'jeans.md'),
    'Casual jeans outfit'
  );
  ...
  await fs.mkdir(path.join(extManifest, '070_look', '4_attire', 'fancy'), { recursive: true });
  ...
  await fs.writeFile(
    path.join(extManifest, '070_look', '4_attire', 'fancy', 'dress.md'),
    'Fancy dress'
  );
...
async function runTests() {
  ...
  try {
    ...
    if (!sections['070_look']) {
      throw new Error('Missing 070_look section');
    }
    ...
    // Test 4: Verify deep nesting (attire with org directories)
    console.log('\nTest 4: Verify deep nesting (attire with org directories)');

    const lookChildren = sections['070_look'].tree.children;
    if (!lookChildren['4_attire']) {
      throw new Error('Missing 4_attire subsection');
    }

    const attireChildren = lookChildren['4_attire'].children;
    if (!attireChildren['casual']) {
      throw new Error('Missing casual org directory');
    }
    if (!attireChildren['fancy']) {
      throw new Error('Missing fancy org directory');
    }

    // Check files in org directories
    if (!attireChildren['casual'].files.includes('jeans')) {
      throw new Error('Missing jeans in casual directory');
    }
    if (!attireChildren['fancy'].files.includes('dress')) {
      throw new Error('Missing dress in fancy directory');
    }
