...
describe('MultiManifest', () => {
  ...
  describe('Basic File Operations', () => {
    ...
    it('should handle partial matching with fuzzy search', async () => {
      const main = await createTempDir();
      await createTestStructure(main, {
        'look/attire/cozy_morning_outfit.md': 'cozy content',
        'look/attire/office_professional.md': 'office content'
      });

      multiManifest = new MultiManifest([main]);

      const file = await multiManifest.findFile('look', 'attire', 'cozy');
      expect(file).toBeTruthy();
      expect(file.filename).toBe('cozy_morning_outfit');
    });
  ...
  describe('Real-world Scenarios', () => {
    it('should handle the attire bug (file in wrong manifest)', async () => {
